...
class BayesianQuadratureModel(object):
    ...
    def __init__(self, x, y, kernel='periodic', obs_noise=False,
                 ntry=10, verbose=False):
        ...
        self.mll = KernelMLL(kernel=kernel, obs_noise=obs_noise)
        # number of ML tries
        self.ntry = ntry
    ...
    def fit(self, iix):
        ...
        self.mu_logS, self.cov_logS, self.theta_logS = self._fit_gp(
            self.xi, np.log(self.yi + 1), "log(S)")
    ...
    def integrate(self, px):
        ...
        m_Z = np.sum(px * self.mean)
        V_Z = np.dot(px * self.mu_S, np.dot(self.cov_logS, px * self.mu_S))
        return m_Z, V_Z
