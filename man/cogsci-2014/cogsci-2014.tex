...
\begin{abstract}
  ...
  human performance. \TODO{None of the models were a particularly
    ...
    provides the foundation necessary to begin answering this
    question.}
...
they make these decisions (see Figure~\ref{fig:model-traces} for
...
Equation~\ref{eq:lh-ratio} while still obeying the constraint of
sequential rotations. Based on evidence that people use ``active
sampling'' \cite<e.g.,>{Nelson2007,Gureckis:2012gu}, we hypothesize a
model that actively monitors the usefulness of the rotations it is
performing. Instead of searching for a maximum, we maintain a
probability distribution over our \textit{estimate} of
Equation~\ref{eq:lh-ratio}, and then sample actions which are expected
to improve that estimate.  This strategy has several benefits. First,
it does not make assumptions about the shape or scale of the
similarity function. Second, by choosing to sample places which are
...
\begin{figure*}[t]
  \begin{center}
    ...
    \caption{\textbf{Model vs. human RTs.} Each subplot shows the
      z-scored model response times ($x$-axis) vs. the z-scored human
      response times ($y$-axis). Pearson correlations are shown
      beneath each subplot. The dotted lines are $x=y$.}
...
\Bq{} model.\footnote{The prior also had a small effect on the
  accuracy of the \Hc{} model, because sometimes the local maxima were
  small enough that Equation~\ref{eq:mle-lh-ratio} became dominated by
  the prior.}
...
whether $p>\frac{0.05}{720}$ for the 50\% mark, where the denominator
is a Bonferroni correction for multiple comparisons.
...
To gauge between-participant reliability, we randomly split
participants into two groups, computed the average RT and accuracy per
...
RT, the between-participants correlation was \ExpTimeCorr{}, and for
accuracy, it was \ExpAccuracyCorr{}.
...
\OcTimeCorr{} (Figure~\ref{fig:human-model-scatters}).  However, the
...
between minimum angle and accuracy.
...
there were \BqNumChance{} stimulii for which it was not above chance;
...
\textit{a priori} knowledge that people are unlikely to have about
...
(Figure~\ref{fig:response-time-stimulus} shows curves for one
particular shape; these curves are fairly typical relative to other
