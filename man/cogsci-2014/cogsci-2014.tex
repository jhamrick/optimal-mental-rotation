...
The only model (of which the authors are aware) that has seriously
attempted to address the decision of \textit{what} to simulate is the
parallel-process model of mental rotation by \citeA{Funt:1983wn}. He
determines the direction, axis, and extent of rotation by computing
the objects' principal axes using moments of inertia. One object is
then rotated through the target rotation, and is checked against the
other object for congruency. However, this approach assumes that the
corresponding points on the two objects can be easily identified,
...
If people are not performing single-trajectory rotations, what might
they be doing? To answer this question, we perform a ``rational
analysis'' of the problem of mental rotation
...
that mental images must be transformed in an analog manner (or in a
way that is at least approximately analog), and that mental images are
...
minimum angle. However, it relies on assumptions of \textit{a priori}
knowledge (either of the minimum angle or the correspondence of points
between shapes). Noting that a rational solution need not necessarily
maintain a single trajectory of rotation, we explore an alternative
rational model, which--rather than assuming the angle of rotation
\textit{a priori}--engages in an ``active sampling'' strategy.
...
% As expected, the minimum angle of rotation was significantly
...
% models performed perfectly, with 100\% accuracy. The \Bq{} model was
...
\begin{figure*}[t]
  \begin{center}
    \subfloat{\includegraphics[width=0.33\textwidth]{../../figures/D/theta_time_corrs.pdf}}
    \subfloat{\includegraphics[width=0.33\textwidth]{../../figures/D/theta_accuracy_corrs.pdf}}
    \subfloat{\includegraphics[width=0.33\textwidth]{../../figures/D/response_time_corrs.pdf}}
    \caption{}
