...
\begin{abstract}
  ...
  human performance. We found that three of the models, which were
  based on previous hypotheses in the literature, were unable to
  account for several aspects of human behavior. The fourth, which is
  a model based on the idea of \textit{active sampling}
  \cite<e.g.,>{Gureckis:2012gu}, provides plausible explanations where
  the other models do not. Based on these results, we suggest that the
  question of ``what to simulate?'' is more difficult than has
  previously been assumed, and that an active learning approach holds
  promise for uncovering the answer.
...
\begin{equation}
  p(h\ \vert\ X_a, X_b) \propto \int p(X_b\ \vert\ X_a, \theta, h)p(h)p(\theta)\dif\theta,
  \label{eq:posterior}
\end{equation}
...
it does not make assumptions about the scale of the similarity
...
$\bar{S}_h:=\exp(\mu_h)$.\footnote{Note that the approach used here
  ...
  \cite{Duvenaud:2013td}.} Then, from \citeA{Duvenaud:2013td}, we have
...
milliseconds; the full histogram of RTs can be seen in
