...
Previous models of mental rotation have, for the most part, not
addressed these questions. \citeA{Kosslyn:1977tv} focus primarily on
simulating the visual simulation buffer rather than the decision of
\textit{what} should be simulated. Similarly, \citeA{Glasgow:1992tj}
are more concerned with the representation of imagery than the
process. Although \citeA{Anderson1978} emphasizes the importance of
considering both representation and process, he dismisses the problem
of determining the direction and axis of rotation as merely a
``technical difficulty''.

The only model (of which the authors are aware) that has more
seriously attempted to address these concerns is the parallel-process
model of mental rotation by \citeA{Funt:1983wn}. He determines the
direction and axis of rotation using the method-of-moments method;
once this target rotation computed, his model rotates one object until
the target rotation is reached, and then compares the two to see if
they are congruent.

More recently, evidence has come to light that people \textit{do not}
actually rotate until the objects are congruent. Eye-tracking studies
suggest people pay attention to certain salient features of the
object, rather than a holistic representation \cite{Just1976}, and 
\citeA{Yuille:1982tx} e.g. rotating away from near-perfect matches \cite{Gardony:2013gn}
...
likely, the quantity of interest is a ratio $\ell:=p(h=0\ \vert\ X_a,
X_b) / p(h=1\ \vert\ X_a, X_b)$ which (assuming the two hypotheses are
equally likely \textit{a priori}, and that all rotations are equally
likely) simplifies to:
\begin{equation}
...
point where the shapes are aligned.
...
``match''. In the current formulation of the problem, we know the
exact geometry of the shapes, and know that a linear transformation
exists which will perfectly align the shapes. However, more generally,
...
cases, the problem of choosing a threshold becomes much harder: it is
not clear how to choose a reasonable threshold \textit{a priori}, as
the global optimum will depend on many factors. Indeed, shape matching
is a non-trivial problem in computer vision, requiring sophisticated
algorithms to achieve reasonable performance
\cite<e.g.,>{Belongie:2002tj,Sebastian:2003vm}.

Thus, the \Th{} model will not be robust beyond this domain. One
obvious alternative would be to perform an exhaustive search or use a
global optimisation strategy. However, this would not result in the
linear response time found by \citeA{Shepard1971}. A second
alternative (which would potentially demonstrate the linear response
time result) could instead just perform a Hill Climbing (\Hc{}) search
for a \textit{local} maximum. Similar to the \Th{} model, we use the
(local) MLE estimate of $\theta$, as in Eq.~\ref{eq:mle-lh-ratio}.
...
estimate. \TODO{Really need to motivate this better: why do we need
  something so complex? Talk about how it's robust to the shape of the
  function, the scale, noise, etc. It's complicated, but it should be
  able to handle these things, so let's try it and see if it works at
  all. If it does, we can work towards somethign more succinct in the
  future.}
