...
\begin{figure}[t]
  ...
  \caption{\textbf{Rotated shapes and their similarity.}  \textbf{(a)}
    ...
    slightly greater than $\pi$, corresponding to the global maximum
...
\textit{na\"ive} model prior to estimating $Z$.
...
\cite{Osborne:2012tm}:
\begin{equation}
  \mu_Z=E[Z\ \vert \ \log S] \approx \int_R \mu_{S}(1 + \mu_\Delta) p(R)\ \mathrm{d}R 
  \label{eq:bq-Z-mean}
\end{equation}
...
$Z$ is given by:
\begin{equation}
  \sigma_Z=\mathrm{Var}(Z\ \vert \ S, \log S) = \int_R\int_{R^\prime} \mu_S(R)\mu_S(R^\prime) \Sigma_{\log S}(R, R^\prime)p(R)p(R^\prime)\ \mathrm{d}R\ \mathrm{d}R^\prime
  \label{eq:bq-Z-var}
\end{equation}
...
\begin{figure}[t]
  ...
  \caption{\textbf{Accuracy in estimating $Z$.} \TODO{} \TODO{use
      different markers for $h_0$ and $h_1$ points}}
  \label{fig:accuracy}
\end{figure}
