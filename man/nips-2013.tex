...
\cite{Shepard1971}. There is strong evidence for such \textit{mental
  imagery} or \textit{mental simulation}: we can imagine
three-dimensional objects in our minds and manipulate them, to a
certain extent, as if they were real \cite{Kosslyn:2009tj}.  However,
the use of mental simulation is predicated on determining appropriate
parameters to give the simulation, and people's cognitive constraints
may furthermore place limitations on the duration or precision of
simulation. One hypothesis for how these issues are handled argues
that people use a rational solution, meaning that it is optimal under
existing constraints \cite{Lieder:2012wg,Vul:2009wy,Griffiths2012a},
such as processing speed, memory, or perceptual acuity.
...
we need only consider $2n$ mappings, $M$, of the $n$ vertices (we
assume uncertainty for which is the ``first'' vertex, and then which
of its two neighbors is the ``second''). Thus, the possible orderings
are of the form $M=\lbrace{}0, 1, \ldots{}, n\rbrace{}$,
$M=\lbrace{}n, 0, \ldots{}, n-1\rbrace{}$, and so on. This gives us an
explicit form for the similarity function:
\begin{equation}
...
prior over the log likelihood\footnote{In practice, as in
  ...
  parameters, we set the output scale $h_S=\sqrt{0.1}$ and $h_{\log
...
\TODO{To choose $\mathbf{R}$}, we pick an initial direction of
rotation which results in the higher value of $S$, and then continue
rotating in that direction until the variance in Equation
\ref{eq:bq-Z-var} is low enough that we are confident about the
...
% I think it would be compelling to add a second row to Equation 5
% where you show the averages and s.e. at each rotation for many noisy
% runs of the algorithm (possibly on different stimuli). If you can
% show that the average is a linear function, in a way that looks like
% the classic behavioral data, that will be compelling.
...
which had between 3 and 6 vertices, inclusive, (e.g. $X_a$ in Figure
\ref{fig:stimuli}). For each shape, we computed 18 ``different'' and
18 ``same'' stimuli pairs, with $R$ spaced at $20^\prime$ increments
between 0 and 360. ``Different'' pairs were created by reflecting
$X_a$ across the $y$-axis and rotating it by $R$; for ``same'' pairs,
$X_a$ was rotated by $R$. \TODO{need to mention that shapes were
  chosen to be discriminable by the gold standard}
...
\item \textit{Rotations}: for those ``same'' pairs which the model
  gets correct, how correlated are the true angles of rotation with
  the rotations performed by the model?  This is defined to be the
  Pearson's correlation coefficient $\rho$ between $R$ and $\vert
...
The mean error of the gold standard model was $\GSME{}$, indicating
that $Z$ is a sufficient approximation of $\int_R p(X_b\ \vert\
X_R)p(R)\ \mathrm{d}R$ for these particular stimuli.
...
$h_0$ stimuli ($\NaiveMEdiff{}$), where the asymmetric linear
interpolation gives an overestimate of $Z$ (e.g., in Figure
\ref{fig:li}). There is some error from $h_1$ stimuli as well
($\NaiveMEsame{}$), which happens when the hill climbing search finds
a local maxima and terminates prematurely, thus resulting in a low
estimate of $Z$.

The correlation between the average number of rotations computed by
the \naive{} model and the true angle of rotation was
$\Naivecorr{}$. The shape of the data is more interesting (see Figure
\ref{fig:rotations}, top left panel): the \naive{} model actually
corresponds extremeley well to the true angle of rotation for
$R<\frac{\pi}{2}$ ($\Naivecorrsm{}$). This is unsurprising, because
the closer the true angle is to zero, the less the model has to
rotate, and the less likely it will get stuck on local maxima. Thus,
it is more likely to locate the global maximum, which corresponds to
the true angle of rotation. For $R>\frac{\pi}{2}$, we see an
increasing tendency to under-rotate due to getting stuck on local
maxima, as well as a tendency to over-rotate if the wrong rotation
direction was chosen.

The \naive{} model's error on $h_0$ stimuli is reflected in its
accuracy in estimating $Z$ ($\NaiveMSE{}$). Figure \ref{fig:accuracy}
(left panel) shows individual estimates for each stimulus, color-coded
by true hypothesis. The model overestimates $Z$ for most of the $h_0$
and $h_1$ stimuli, and underestimates several of the $h_1$ stimuli as
well.

\paragraph{Parametric (Von Mises)}

The parametric model is also fairly inaccurate ($\VMME{}$), making
more errors on $h_1$ stimuli ($\VMMEsame{}$) than $h_0$ stimuli
($\VMMEdiff{}$). This is because the Von Mises distribution only has a
single peak and thus will underestimate $Z$ for multimodal similarity
functions. However, it does more accurately estimate $Z$ for stimuli
that the \naive{} model would overestimate ($\VMMSE{}$). This
underestimation is severe enough that in most cases where it gets
stuck on a local maxima, it produces the incorrect judgment. So, the
correlation for \textit{correct} responses is actually improved
($\VMcorr{}$) and would be near perfect if the correct diretion of
rotation was initially chosen (see Figure \ref{fig:rotations}, top
center).


\paragraph{Nonparametric (Bayesian Quadrature)}

The nonparametric model is much more accurate in choosing the correct
hypothesis ($\BQME{}$), erring approximately equally for both types of
stimuli. We note that this is very close to the 3.2\% error rate
reported by \cite{Shepard1971}, though further experimentation is
necessary to determine whether the types of errors people make align
...
\ref{fig:rotations}, bottom right). This is because the nonparametric
model does not get stuck as easily on local optima: it will continue
rotating until it is confident that its estimate of $Z$ is
accurate. Indeed, its active attempts to obtain an accurate estimate
of $Z$ are largely succeessful ($\BQMSE{}$). Moreover, it has the
capacity to ``restart'' and can recover from initially rotating in the
incorrect direction, as illustrated by the fewer number of extreme
over-rotations in Figure \ref{fig:rotations} (top right).
